...
#define SMB358_TERMINATION_CURRENT_VALUE_60mA    BIT(1)
...
static int smb358_pre_config()
{
    ...
    /* set termination current: 60mA */
    ret = smb345_masked_write(smb345_dev->client,
        CHG_CURRENT_REG,
        SMB358_TERMINATION_CURRENT_MASK,
        SMB358_TERMINATION_CURRENT_VALUE_60mA);
    if (ret < 0)
        goto fail;
...
static int smb345_recharge_voltage(charger_jeita_state_t charger_jeita_state)
{
    ...
    else if (charger_jeita_state == RTS)
...
bool is_battery_id_51k() { return check_valid_battery_id(ID51K); }
bool is_battery_id_100k() { return check_valid_battery_id(ID100K); }
bool is_battery_id_10k() { return check_valid_battery_id(ID10K); }
